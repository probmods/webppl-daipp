...
```{r}
d.raw = #read.csv("model_vae,query_mu,zDim_3,M_60000,steps_1500,seed_0.csv") %>%
  read.csv("model_vae,query_mu,zDim_3,M_60000,steps_500,seed_20.csv") %>%
...
classify = function(k = 1) {
  knn.res = get.knn(data = d[,3:5], k = k)
...
sapply(21:40, function(k) { mean(d$label == classify(k = k)) })
```
