...
```{r}
knn = function(k.max = 5) {
  dat = d %>% select(starts_with("z"))
  get.knn(data = dat, k = k.max)
}
knn.res = knn(5)
```

```{r}
classify = function(k, knn.res) {
  all.nn.indices = as.matrix(knn.res$nn.index[,1:k])
  apply(all.nn.indices,
        ...
        function(nn.indices) {
          ...
          # optimized:
          which.max(table(d[nn.indices,'label'])) - 1
        })
}

system.time(foo <- classifier.accuracy(1, knn.res))
sapply(1:1, function(k) { mean(d$label == clasify(k, knn.res)) %>% round(3) })
```
