...
To evaluate the guide program's posterior prediction ability, we use the guide to sample latent causes given an observed set of effects from the test set and then evaluate the negative log-likelihod of those same effects given the sampled latents. This process essentially uses the guide program to `reconstruct' the observations.
Figure~\ref{fig:qmrResults} Right shows the results of this evaluation. Test set observations have around twice the log-likelihood under predictions from the fully variance-reduced program (\emph{+ baselines}) than the basic amortized guide program.

\begin{figure}[!ht]
\begin{minipage}{0.6\linewidth}
\centering
\includegraphics[width=\linewidth]{figs/results/qmr/elboProgress.pdf}
\end{minipage}
%
...
\end{minipage}
\caption{Performance of a QMR-DT model. \emph{(Left)} ELBo optimization progress during training. \emph{(Right)} Negative log-likelihood of test set observations under latent variables sampled from the guide program.}
